...
| Shots fired by one Capture press (papyri) | `papyri.calibration.shot_count_for` (`flatfieldShotCount` key; which targets shoot a series is `CalSpec.multi_shot` / `calibration_layout.is_multi_shot`) | read the key inline, or loop capture requests at a call site — the series must run through `PapyriMainWindow._advance_capture_series` so aborts and the button gate stay in sync |
